...
bool shoot(Tanque *t)
{
	...
	// set start values
	t->bullets[i].angulo = t->angCanhao;
	t->bullets[i].direcao = t->direccao + t->angTorre;

	t->bullets[i].x0 = t->x + (COMPRIMENTO_TORRE / 2.0 + (COMPRIMENTO_CANHAO * cos(RAD(t->angCanhao))))* cos(RAD(t->direccao + t->angTorre + 90));
	t->bullets[i].y0 = t->y + (COMPRIMENTO_TORRE / 2.0 + (COMPRIMENTO_CANHAO * cos(RAD(t->angCanhao))))* sin(RAD(t->direccao + t->angTorre + 90));
	t->bullets[i].z0 = ALTURA_BASE + (ALTURA_TORRE / 2.0) + COMPRIMENTO_CANHAO * sin(RAD(t->angCanhao));

	t->bullets[i].t = 0;
	t->bullets[i].IsAlive = true;
